...
contract Auction {
    ...
    function bid(uint256 _auctionId, uint256 _amount) external payable nonReentrant mustRegister returns (bool) {
        ...
        // Track participation per auction
        if (!hasParticipated[msg.sender][_auctionId]) {
            user.totalAuctionParticipated++;
            hasParticipated[msg.sender][_auctionId] = true;
        }
    ...
    function auctionEnd(uint256 _auctionId) external mustRegister nonReentrant returns (bool){
        ...
        if (newUser.totalAuctionParticipated > 0) {
            newUser.winrate = (newUser.totalWinningBids * 100) / newUser.totalAuctionParticipated;
        }
    ...
    function getAllUsersStats() external view returns (
        ...
        uint256[] memory totalBids = new uint256[](userCount);
        uint256[] memory totalSpent = new uint256[](userCount);

        for (uint256 i = 0; i < userCount; i++) {
            address userAddr = userIndex[i];
            UserStruct storage user = users[userAddr];
            userAddresses[i] = userAddr;
            totalBids[i] = user.totalBid;
            totalSpent[i] = user.totalValueBid;
        }

        return (userAddresses, totalBids, totalSpent);
    }
}